...
for filename in files:
    ...
    content = content.replace('<body>', wrapper_start)

    # 2. Close wrapper and fix the footer
    footer_fixed = '</div>\n<footer style="position: fixed; bottom: 0; left: 0; width: 100%; z-index: 1;">'
    content = content.replace('<footer>', footer_fixed)

    # 3. Inject JS to handle the dynamic margin
    js_script = """<script>
...
print("Parallax Reveal Footer applied successfully!")
